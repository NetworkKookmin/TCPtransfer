...
while(1){ 
        ...
        if(!strcmp(strRatecurr,command)){
            printf("SendBUFSIZE = %d  , RecvBUFSIZE = %d \n", SendBUFSIZE,RecvBUFSIZE);
            continue;
...
////////////////////////////// put
        ...
        if(!strcmp(strSendrate,command)){
            write(sock,strSendrate,20);
            strcpy(rateCount,file_name);
            SendBUFSIZE=atoi(rateCount);
            printf("OK\n");
            write(sock,rateCount,20);

        }
        if(!strcmp(strRecvrate,command)){

            write(sock,strRecvrate,20);

            strcpy(rateCount,file_name);
            RecvBUFSIZE=atoi(rateCount);
            printf("OK\n");
            write(sock,rateCount,20);
        ...
        if(!strcmp(strPut,command)){  // put 입력시 
         ...
         while(1){             
            ...
            numread = fread(buf,1,SendBUFSIZE,fp); // fp파일을 읽어서 buf에 저장하고 그 저장한 바이트 만큼 numread에 int값으로 저장 
...
void printPercent(double percent, int sumfileSize,int fileSize,int* count)
{
    if(percent<10 && *count==10000){            printf("[*         ] ( %d% ) %dMB/%dMB\r",(int)percent,sumfileSize/1000000,fileSize/1000000);fflush(stdout); 
 *count=0;} 
            else if(percent<20 && *count==10000){       printf("[**        ] (%d% ) %dMB/%dMB\r",(int)percent,sumfileSize/1000000,fileSize/1000000);fflush(stdout); 
 *count=0;} 
            else if(percent<30 && *count==10000){       printf("[***       ] (%d% ) %dMB/%dMB\r",(int)percent,sumfileSize/1000000,fileSize/1000000);fflush(stdout); 
 *count=0;} 
            else if(percent<40 && *count==10000){       printf("[****      ] (%d% ) %dMB/%dMB\r",(int)percent,sumfileSize/1000000,fileSize/1000000);fflush(stdout); 
 *count=0;} 
            else if(percent<50 && *count==10000){       printf("[*****     ] (%d% ) %dMB/%dMB\r",(int)percent,sumfileSize/1000000,fileSize/1000000);fflush(stdout); 
 *count=0;} 
            else if(percent<60 && *count==10000){       printf("[******    ] (%d% ) %dMB/%dMB\r",(int)percent,sumfileSize/1000000,fileSize/1000000);fflush(stdout); 
 *count=0;} 
            else if(percent<70 && *count==10000){       printf("[*******   ] (%d% ) %dMB/%dMB\r",(int)percent,sumfileSize/1000000,fileSize/1000000);fflush(stdout); 
 *count=0;} 
            else if(percent<80 && *count==10000){       printf("[********  ] (%d% ) %dMB/%dMB\r",(int)percent,sumfileSize/1000000,fileSize/1000000);fflush(stdout); 
 *count=0;} 
            else if(percent<100 && *count==10000){       printf("[********* ] (%d% ) %dMB/%dMB\r",(int)percent,sumfileSize/1000000,fileSize/1000000);fflush(stdout); 
 *count=0;}else {}
